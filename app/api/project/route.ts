...
import { WIREFRAME_AI_CHAT_PROMPT, WIREFRAME_AI_INTENT_PROMPT, WEB_ANALYSIS_PROMPT, WEB_GENERATION_PROMPT } from "@/lib/prompt";
...
- Context from previous pages : ${previousPagesContext}
  ...
  const summaryResult = await callAIWithRetry(() => insforge.ai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You are Wireframe.ai, an AI web design agent. You just finished building pages.
          ...
          if (project?.title) {
            ...
            const result = await callAIWithRetry(() => insforge.ai.chat.completions.create({
              ...
              messages: [
                {
                  role: "system",
                  content: WIREFRAME_AI_INTENT_PROMPT,
                },
                {
                  role: "user",
            ...
            if (classification.intent === "chat") {
              const chatResult = await callAIWithRetry(() => insforge.ai.chat.completions.create({
                ...
                messages: [
                  {
                    role: "system",
                    content: WIREFRAME_AI_CHAT_PROMPT
                  },
                  ...modelMessages
                ],
                stream: true,
